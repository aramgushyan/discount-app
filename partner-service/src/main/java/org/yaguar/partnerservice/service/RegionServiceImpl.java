...
public class RegionServiceImpl implements RegionServiceInterface {
    ...
    public Result<List<RegionResponseShort>> findAllRegions() {
        ...
        return new Result<>(regionMapper.toResponseShortList(regions), null, HttpStatus.OK);
    }
    ...
    public Result<RegionResponseLong> findRegionById(Long id) {
        return regionRepository.findById(id)
                .map(regionEntity -> new Result<>(
                        regionMapper.toResponseLong(regionEntity), null, HttpStatus.OK))
                .orElseGet(() -> new Result<>(
                        null, "Region not found", HttpStatus.NOT_FOUND));
    }
    ...
    public Result<Long> createRegion(RegionAddRequest regionAddRequest) {
        ...
        return new Result<>(regionRepository.save(region).getId(), null, HttpStatus.CREATED);
    }
    ...
    public Result<Void> deleteRegionById(Long id) {
        ...
        if (optionalRegion.isEmpty()) {
            return new Result<>(null, "Region not found", HttpStatus.NOT_FOUND);
        }
